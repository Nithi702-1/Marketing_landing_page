...
/** Standard container — 60 px horizontal gutter on ≥ md, 30 px on mobile */
...
/** Wide / breakout container — 30 px gutter at all breakpoints for a full-bleed feel */
...
// ─── Services section feature data (6 cards → 3 × 2 equal grid) ──────────────
const SERVICE_FEATURES = [
  {
    title: "Stay Informed",
    desc: "Access the latest share prices, press releases, events, and financial results instantly. Dive into dividend history, analyst consensus, and key financials.",
  },
  {
    title: "Interactive Charting",
    desc: "Analyze historical performance with advanced charting tools. Compare against indices, peers, and track long-term growth with ease.",
  },
  {
    title: "Curated Watchlist",
    desc: "Monitor stock performance with a curated list of peers and personal watchlists synced to your user profile across any device.",
  },
  {
    title: "Never Miss Updates",
    desc: "Stay updated with push notifications for reports, news, and events—and add those events directly to your device calendar.",
  },
  {
    title: "Real-Time Data",
    desc: "Access live market data, price movements, and trade volumes at your fingertips, refreshed in real time throughout the trading day.",
  },
  {
    title: "Document Library",
    desc: "Instantly browse and download annual reports, investor presentations, regulatory filings, and press releases from one central hub.",
  },
];
...
const Navbar = () => {
  ...
  const [activeTab,        setActiveTab]        = useState("");   // "" = no highlight (hero zone)
  const isManualNavRef = useRef(false);                           // suppress spy during click-scroll
  ...
  // ── Scroll-spy — keeps active tab in sync with visible section ─────────────
  useEffect(() => {
    ...
    const updateActive = () => {
      if (isManualNavRef.current) return;   // yield to manual click

      const vh        = window.innerHeight;
      const heroBound = document.getElementById("hero")?.getBoundingClientRect();

      // Keep tabs silent while the hero still dominates the viewport
      if (heroBound && heroBound.bottom > vh * 0.4) {
        setActiveTab("");
        return;
      }

      // Activate the section that covers the most viewport pixels
      let bestTab = "";
      let bestPx  = 0;
  ...
  /** Manual tab click — pin the active state and pause spy for 1 s */
    ...
    <div className="min-h-screen overflow-x-hidden">
      ...
      <header id="hero" className="relative h-[900px] flex items-center overflow-hidden">
        ...
        <Container className="relative z-10 pt-20 h-full">
          <div className="flex flex-row items-end justify-between gap-8 w-full h-full pt-[120px] pb-0 px-0">
            ...
            {/* ── RIGHT: phone mockup — rises from bottom ── */}
            <motion.div
            ...
            >
              <img
                ...
                style={{
                  maxHeight: 760,
                  mixBlendMode: "multiply",        /* blends white BG into the blue hero */
      ...
      <section id="services" className="bg-white" style={{ padding: "72px 0 72px" }}>
        <WideContainer>
          ...
          {/*
           * Bento layout (3-col × 2-row grid):
           *  ┌──────────┬─────────────────────┐
           *  │ Block 1  │      Block 2         │  row 1
           *  │ (anchor) ├──────────┬──────────┤
           *  │          │ Block 3  │ Block 4  │  row 2
           *  └──────────┴──────────┴──────────┘
           */}
          <div
            style={{
          ...
          >
            ...
            {/* ── Block 1: Primary Anchor — col 1, spans both rows ── */}
            <motion.div
            ...
            {/* ── Block 2: Wide Feature — col 2-3, row 1 ── */}
            <motion.div
              initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
            ...
            {/* ── Block 3: Twin Square — col 2, row 2 ── */}
            <motion.div
              initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
            ...
            {/* ── Block 4: Twin Square — col 3, row 2 ── */}
            <motion.div
              initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
      ...
      {/* ══ 4. INVESTOR EXPERIENCE — centred text + glassmorphism box ════════ */}
      <Section id="platform" className="bg-primary-gradient relative overflow-hidden">
        {/* Soft corner light orbs */}
        <div
        ...
        <Container className="relative z-10 text-center">
          ...
          {/* Body text — max 700 px for clean line measure */}
          <motion.p
            {...fadeIn}
          ...
          {/* ── Glassmorphism container with Tab mockup ── */}
          <motion.div
            {...fadeIn}
      ...
      <Section id="cta" className="pb-24">
        <Container>
          ...
          >
            {/* Download_BG overlay on top of the gradient */}
            <img
              src="/Download_BG.png"
              alt=""
              aria-hidden="true"
